...
class ClOrderController extends Controller
{
        ...
        try {
            ...
            if (!$counterparty) {
                return response()->json([
                    'message' => 'К вам не прикреплен контрагент',
                    'data' => [],
                ], 404);
            }

            $query = Order::query()
                ->where('counterparty_id', $counterparty->id)   // заказы только текущего пользователя
                ->with(['limitedItems.product', 'counterparty', 'store'])
                ->withCount('items');

            // фильтрация
            if ($request->filled('store_id')) {
                $query->where('store_id', $request->address_id);
            }
    ...
    {
        $user = $request->user();
        $order = Order::with(['items.product', 'store'])->withCount('items')->findOrFail($id);
        if ($order->counterparty_id !== $user->counterparty->id) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($order->status !== 'new') {
            return response()->json(['message' => 'Редактирование возможно только для новых заказов'], 403);
        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if ($order->status !== 'new') {
            return response()->json(['message' => 'Отмена возможна только для новых заказов'], 403);
        }
